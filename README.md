...
```
input/ 폴더에 음성(.m4a 등) [+ 같은 이름의 이력서] 넣기
   │
   ├─ ① 로컬 Whisper 로 한국어 받아쓰기   (완전 무료, 오프라인)
   ├─ ② Google Gemini 로 양식 항목 추출   (무료 티어, 이력서가 있으면 함께 참고)
...
### 이력서 함께 사용하기 (선택)
음성 파일과 **같은 이름**의 이력서를 `input` 폴더에 함께 넣으면, 이력서의 객관적
정보(기술·회사·재직기간·연봉 등)와 통화 내용(이직 사유·구직 상황)을 종합해 작성합니다.

- 예) `정주호.m4a` + `정주호.pdf`  또는  `정주호.m4a` + `정주호_이력서.pdf`
- 지원 형식: **PDF·이미지(png/jpg)** 는 Gemini가 직접 읽음, **docx·txt** 는 텍스트로 읽음.
- 한글(.hwp)은 직접 지원하지 않습니다 → **PDF로 저장 후** 넣어주세요.
- 이력서를 안 넣으면 전화 내용만으로 작성합니다(기존과 동일).
...
### 더블클릭 실행 (가장 쉬움, Windows)
`input` 폴더에 음성+이력서를 넣고 **`실행.bat` 을 더블클릭**하면, input 폴더의
모든 음성을 한 번에 처리해 `output` 에 결과 `.docx` 를 만듭니다. 처리에 성공한
음성/이력서는 자동으로 `input\done` 폴더로 옮겨져, input 은 다음 작업을 위해
비워집니다. (실패한 파일은 input 에 남아 다시 시도 가능)
